...
async function attemptGenerate(scenario, index, total, successful, failed) {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    try {
      ...
      if (!isValidBuffer(buffer)) throw new Error("Invalid QR buffer");
...
export async function generateFullQrCache(forceComplete = true) {
  ...
  while (pending.length > 0 && cycle < 10) {
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      queue.add(() => attemptGenerate(scenario, index, totalCount, successful, failed));
    }
  ...
  console.log(`✅ Total successful: ${totalSuccess = successful.size}`);
  console.log(`❌ Total failed: ${totalFailures = totalCount - totalSuccess}`);
        ...
        queue.add(async () => {
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              ...
              if (!isValidBuffer(buffer)) throw new Error(`Invalid buffer on attempt ${attempt + 1}`);
              await fs.writeFile(filePath, buffer);
              console.log(`✅ Regenerated: ${path.basename(filePath)}`);
              return;
            } catch (err) {
              console.warn(`⏳ Retry #${attempt + 1} for ${path.basename(filePath)} → ${err.message}`);
              await sleep(1000 * (attempt + 1));
            }
          }
          console.warn(`❌ Final failure after retries: ${path.basename(filePath)}`);
        });
      }
