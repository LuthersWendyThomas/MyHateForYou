...
async function renderStep(bot, uid, step, userMessages) {
  ...
  try {
    switch (step) {
      case 1:
        return sendKeyboard(bot, uid, "🌍 *Choose your region:*", getRegionKeyboard(), userMessages, { parse_mode: "Markdown" });

      case 1.2:
        return sendKeyboard(bot, uid, "🏙️ *Select your city:*", getCityKeyboard(session.region), userMessages, { parse_mode: "Markdown" });
      ...
      case 2: {
        ...
        return sendKeyboard(bot, uid, `🚚 *Choose delivery method for ${city}:*`, keyboard, userMessages, { parse_mode: "Markdown" });
      }

      case 2.1:
        return sendKeyboard(bot, uid, "🎁 *Have a promo code?*", [
      ...
      case 7: {
        const { product, quantity, unitPrice, deliveryFee, promoCode, appliedDiscount, currency, totalPrice } = session;
        const summary =
          `🧾 *Order Summary:*\n\n` +
      ...
      case 8:
        return sendKeyboard(bot, uid, "⏳ *Waiting for payment...*", [
          [{ text: MENU_BUTTONS.CONFIRM.text }],
...
export async function handleStep(bot, id, text, userMessages, ctx = {}) {
  ...
  if (!input) return renderStep(bot, uid, session.step, userMessages);
